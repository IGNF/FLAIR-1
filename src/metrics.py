...
def generate_miou(truth_images: str, path_pred: str) -> list:
    ...
    truth_images = sorted(truth_images, key=lambda x: int(x.split('_')[-1][:-4]))
    preds_images  = sorted(list(get_data_paths(Path(path_pred), 'PRED*.tif')), key=lambda x: int(x.split('_')[-1][:-4]))

    try: 
        assert len(truth_images) == len(preds_images), '[WARNING !] mismatch number of predictions and test files.'
        assert truth_images[0][-10:-4] == preds_images[0][-10:-4], '[WARNING !] unsorted images and masks found ! 1st MSK-PRED not correponding.'
        assert truth_images[-1][-10:-4] == preds_images[-1][-10:-4], '[WARNING !] unsorted images and masks found ! Last MSK-PRED not correponding.'
