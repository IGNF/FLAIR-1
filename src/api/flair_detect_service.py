...
from src.api.handle_files import download_gcs_folder, download_file, upload_file
...
def get_requested_model(model: SupportedModel,
                        client: Client,
                        data_folder : str = DATA_FOLDER) -> Tuple[FlairModel, str]:
...
def download_file_to_process(
    ...
    try:
        ...
        raise HTTPException(
            status_code=403,
            detail=f"Permission error during download of blob {image_blob_path} "
                   f"in bucket {image_bucket_name} : {str(e)}"
        )
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Unexpected exception during download of blob {image_blob_path} "
                   f"in bucket {image_bucket_name} : {str(e)}"
        )

    logger.info("Blob %s in bucket %s downloaded at %s",
                image_blob_path,
                image_bucket_name,
                image_local_path)
...
def run_prediction(prediction_config_path: str):
    try:
        result = run(["flair-detect", "--conf", prediction_config_path],
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Unexpected exception during flair-detect script : {str(e)}"
        )

    logger.info("Prediction with config %s done with success",
...
def upload_result_to_bucket(
    ...
    try:
        upload_file(bucket_name=output_bucket_name,
        ...
        raise HTTPException(
            status_code=404,
            detail=f"Bucket {output_bucket_name} for upload not found : {str(e)}"
        )
    except Forbidden as e:
        raise HTTPException(
            status_code=403,
            detail=f"Permission error during upload of blob {output_blob_path} "
                   f"in bucket {output_bucket_name} : {str(e)}"
        )
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Unexpected exception during upload of blob {output_blob_path} "
                   f"in bucket {output_bucket_name} : {str(e)}"
        )
